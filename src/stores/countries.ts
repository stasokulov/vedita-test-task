...
export const useCountriesStore = defineStore('countries', () => {
  ...
  const activeCountryName = ref<string>('')

  // Actions
  const fetchCountries = async () => {
    countries.value = countriesArr
  }
  const setActiveCountryName = async (name: string) => {
    activeCountryName.value = name
  }

  // Getters
  const getActiveCountry: ComputedRef<ICountry | undefined> = computed(() => countries.value.find(country => country.name === activeCountryName.value))

  return { countries, activeCountryName, fetchCountries, setActiveCountryName, getActiveCountry }
})
